...
<article class="uk-card uk-card-default uk-card-body">
    ...
    <p>
        <p><?=  $premierMessage->getContenue() ?></p>
        <p>
            Créer par <strong><?=  $premierMessage->getUtilisateur() ?>, </strong> <?=  $premierMessage->getCreatedAt('d/m/Y H:i:s') ?>
        </p>
    </p>
    <div>
        <p><a class="uk-button uk-button-small uk-button-primary" href="?ctrl=forum&action=addMessage&id=<?=  Session::get("user")->getId() ?>">Répondre</a></p>
    </div>
</article>
 ...
 <table id='adminTable' class='uk-table uk-table-hover uk-table-divider'>
    ...
    <tbody>
        ...
        ?>       
            <tr>
                <td><?=  $message->getContenue() ?></td>
                <td><?=  $message->getCreatedAt('d/m/Y H:i:s') ?> </td>
